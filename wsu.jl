#UAI 2023
#Calculate the return of the WSU algorithm
...
function get_max_action(s,state_space,action_space,  model_space, update_state_value, 
    ...
    max_action = 1
    w = 1/length(model_space) # uniform model weights
    ...
    for a in action_space
        ...
        if pro > 1e-15 
            for m in model_space
                ...
                weighted_models_rewards += w* one_model_reward
            end
    ...
    while t>= 1
        ...
        for m in model_space
            for s in state_space
                ...
                for s_next in state_space
                        future_reward = future_reward +  p[s,pi_state[t,s],s_next,m] * 
                ...
                for  s_next in state_space
                        immediate_reward = immediate_reward +  r[s,pi_state[t,s],s_next,m] * p[s,pi_state[t,s],s_next,m]
                end
    ...
    while t>= 1
        for m in model_space
            for s in state_space
                ...
                for  s_next in state_space
                        future_reward += p[s,policy[t,s],s_next,m] * update_state_value[m,s_next,t+1]
                end
              
                #Calculate the immediate reward  
                for  s_next in state_space
                        immediate_reward += r[s,policy[t,s],s_next,m] * p[s,policy[t,s],s_next,m]
                end 
...
function calculate_total_reward(values_of_states,ini_states,model_space,state_space)
    ...
    all_mp = joinpath(@__DIR__,"resultfiles","all_returns_WSU_.csv")
    ...
    for domain in domains
       ...
       if(domain == 'i')
        ...
        while T > 1
                ...
                # Initialize state values of all models at step T+1   
                update_state_value_test =  zeros((length(model_space_test), length(state_space_test),T+2))
            
                # result is the state values of all models at time step 1
        ...
        # # write results to file
        mp = joinpath(@__DIR__,"resultfiles","WSU_$domain T_$initial_T.csv")
